...
-- Get the formatter command based on configuration
local function get_formatter_command(config)
  if config.formatter.command then
    if type(config.formatter.command) == "string" then
      return { config.formatter.command, "--language", config.formatter.language }
    else
      return config.formatter.command
    end
  end
...
function M.format_sql_in_buffer(bufnr, config)
